...
void callback_dt(const ImageConstPtr& in_image, const ImageConstPtr& in_mask)
{
  ...
  float fv_elevation = 26.5; // 45.0
  float fv_azimuth = 360.0;
  float n_elevation = 64.0; //128.0
  float n_azimuth = 1568; //2048.0
  float max_elevation = 2.0; //22.5
  float max_azimuth = 184.0;
  ...
  for (int i = 0;i<img_range.rows; i++){
      for (int j = 0;j<img_range.cols; j++){
        ...
        float ang_h = max_elevation - (fv_elevation/n_elevation)*i;
        ang_h = ang_h*M_PI/180.0;
        float ang_w = max_azimuth - (fv_azimuth/n_azimuth)*j;
  ...
  cloud_out->height = 1;
  cloud_out->header.frame_id = "velo_link"; //"os_sensor";
